...
def main():
    ...
    if result.returncode != 0:
        logger.error(
            f"Failed to checkout branch {branch_name} in worktree: {result.stderr}"
        )
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id, "ops", f"❌ Failed to checkout branch {branch_name} in worktree"
            ),
        )
        sys.exit(1)
    ...
    make_issue_comment(
        ...
        format_issue_message(
            adw_id,
            "ops",
            f"✅ Starting isolated implementation phase\n"
    ...
    make_issue_comment(
        ...
        format_issue_message(
            adw_id,
            AGENT_IMPLEMENTOR,
            "✅ Implementing solution in isolated environment",
        ),
    )

    implement_response = implement_plan(
        str(plan_file), adw_id, logger, working_dir=worktree_path
    )
    ...
    if not issue_command or issue_command not in ["/chore", "/bug", "/feature"]:
        logger.info("No valid issue classification in state, running classify_issue")
        from adw_modules.workflow_ops import classify_issue

        classified_command, error = classify_issue(issue, adw_id, logger)
        if error or not classified_command:
            logger.error(f"Error classifying issue: {error}")
            # Default to feature if classification fails
            issue_command = "/feature"
            logger.warning("Defaulting to /feature after classification error")
        else:
            issue_command = classified_command
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(
            adw_id, "ops", "✅ Isolated implementation phase completed"
        ),
    )
